 ...
 * Circuit's Queue. Each Script creates a Capture and calls the parent Conduit's
 * processEmission() method. This ensures all Conduits share the Circuit's single-threaded
 * execution model ("Virtual CPU Core" design principle).
 *
...
public class PipeImpl<E> implements Pipe<E> {
    ...
    private final ConduitImpl<?, E> parentConduit; // Parent Conduit to call processEmission()
     ...
     * @param parentConduit the parent Conduit whose processEmission() will be called
     */
    public PipeImpl(Queue circuitQueue, Subject channelSubject, ConduitImpl<?, E> parentConduit) {
        this(circuitQueue, channelSubject, parentConduit, null);
    }
     ...
     */
    public PipeImpl(Queue circuitQueue, Subject channelSubject, ConduitImpl<?, E> parentConduit, SegmentImpl<E> segment) {
     ...
     * the parent Conduit's processEmission() method. This ensures single-threaded
     * execution within the Circuit domain.
     *
    ...
    private void postScript(E value) {
        ...
        // Post Script that calls parent Conduit's processEmission()
